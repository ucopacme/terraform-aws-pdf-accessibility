...
# Lambda code is zipped locally from the source repo and deployed directly.
# ═══════════════════════════════════════════════════════════════════════════

# ─── Local zip archives for each UI Lambda ────────────────────────────────

data "archive_file" "post_confirmation" {
  type        = "zip"
  source_dir  = "${var.ui_lambda_source_path}/postConfirmation"
  output_path = "${path.module}/.lambda_zips/postConfirmation.zip"
}

data "archive_file" "update_attributes" {
  type        = "zip"
  source_dir  = "${var.ui_lambda_source_path}/updateAttributes"
  output_path = "${path.module}/.lambda_zips/updateAttributes.zip"
}

data "archive_file" "check_or_increment_quota" {
  type        = "zip"
  source_dir  = "${var.ui_lambda_source_path}/checkOrIncrementQuota"
  output_path = "${path.module}/.lambda_zips/checkOrIncrementQuota.zip"
}

data "archive_file" "update_attributes_groups" {
  type        = "zip"
  source_dir  = "${var.ui_lambda_source_path}/UpdateAttributesGroups"
  output_path = "${path.module}/.lambda_zips/UpdateAttributesGroups.zip"
}

data "archive_file" "pre_sign_up" {
  type        = "zip"
  source_dir  = "${var.ui_lambda_source_path}/preSignUp"
  output_path = "${path.module}/.lambda_zips/preSignUp.zip"
}
