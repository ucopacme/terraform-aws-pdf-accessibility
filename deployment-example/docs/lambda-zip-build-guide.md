...
All zips go to your deployment directory:
```
<YOUR-DEPLOYMENT-DIR>/lambda/zip/
```
...
# Deploy
cp /tmp/pdf2html.zip <YOUR-DEPLOYMENT-DIR>/lambda/zip/pdf2html.zip
...
# Deploy
cp /tmp/${ZIP_NAME}.zip <YOUR-DEPLOYMENT-DIR>/lambda/zip/${ZIP_NAME}.zip
...
cp /tmp/title-generator.zip <YOUR-DEPLOYMENT-DIR>/lambda/zip/title-generator.zip
...
cp /tmp/pdf-splitter.zip <YOUR-DEPLOYMENT-DIR>/lambda/zip/pdf-splitter.zip
...
cp target/PDFMergerLambda-1.0-SNAPSHOT.jar \
  <YOUR-DEPLOYMENT-DIR>/lambda/pdf-merger/PDFMergerLambda-1.0-SNAPSHOT.jar
```
